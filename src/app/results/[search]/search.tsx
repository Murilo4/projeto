...
const ResultsPage: React.FC = () => {
  ...
  const searchText = search?.slice(9) || '';
  const city = "Franca"
  ...
  const fetchPlaces = useCallback(async (pageNumber: number, minPrice: number, maxPrice: number, minStars: number, maxStars: number, isOpenNow: boolean, selectedTypes: string[]) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/get-all-places/${pageNumber}/?${`${searchQuery}&city=${city}&lowerPrice=${minPrice}&higherPrice=${maxPrice}&minStars=${minStars}&maxStarts=${maxStars}&openNow=${isOpenNow}&${typesQuery}`}`, {
        method: 'GET',
        headers: {
  ...
  return (
    <>
      ...
      <div
        ...
        className="flex-1 p-4 bg-background-blue rounded-t-lg shadow-md shadow-slate-400 min-h-screen flex flex-col"
      >
        <h2 className="text-2xl font-bold mb-5 content-center justify-center flex">{searchText} em {city} - {state}</h2>
        <div className="inline-grid grid-cols-3 lg:grid-cols-5 gap-2 mb-4">
          <h2 className="flex items-center gap-2 border-2 bg-gray-100 text-base font-medium px-2 py-2 rounded-full shadow border-slate-300">
            <span className="text-green-button">✔</span>{state}
          </h2>
        ...
        <div className="space-y-5">
          {loader ? (
            ...
            places.map((placeData, index) => (
              <div
                key={index}
                className="border-2 border-white bg-white rounded-lg flex shadow-lg shadow-slate-400  hover:scale-105 transition-transform duration-500"
              >
                {/* Imagem do Restaurante */}
                <img
                  src={placeData.photo ? `http://localhost:8000${placeData.photo}` : "/default-image.png"}
                  alt={`Restaurante ${index + 1}`}
                  className="w-44 h-full rounded-l-md object-cover mr-4 border"
                />
